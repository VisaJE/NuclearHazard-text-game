...
object AdventureGUI extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    def playGame(command: String) {
      if (Storyline.isIntro) {
        playIntro(command)
      }
      else {
        playTurn(command)
    ...
    def updateInfo(info: String) = {
      ...
      this.locationInfo.text = {
        if (Storyline.isIntro) Storyline.introText
        else  this.player.location.fullDescription
      }
